...
server.post('/loadBalance/:idUser', (req, res) => {
    ...
    Wallet.update ({
        ...
        returning : true, where: {userId : req.params.idUser}
    })
    .then(newBalance => {
...
server.put('/:idSender/:idReceiver', (req, res) => {
    ...
    let userReceiver = Wallet.findOne({where: {userId:req.params.idReceiver}})
        ...
        .then(users => {
            ...
            //updates en las dos billeteras
                let receiver = Wallet.update({
                    ...
                    returning: true, where: { userId: req.params.idReceiver }
                });
                let send = Wallet.update({
                    balance: newBalanceSender
                }, {
                    returning:true ,where: { userId: req.params.idSender }
                });
                ...
                Promise.all([send, receiver])
                    .then(promises => {
                        ...
                        .then(transaccion =>
                            res.status(200).json({message: "transaccion N°"+transaccion.id+" realizada con exito!",transaccion} )
                        )
                        .catch(err => res.status(400).json({message: "No se registró el movimiento"}))
                    })

                    .catch(err => { res.status(400).json({message: "No se pudo modificar el saldo", err}) })
        })

    .catch(err => res.status(400).json({message: "Usuario no encontrado! por favor ingrese nuevamente los usuarios", error : err}))
})
...
server.get('/history/:idUser', (req, res) => {
    ...
    let ingresos = Transactions.findAll({where: {idReceiver: req.params.idUser}})
    let decrements = Transactions.findAll({where: {idSender: req.params.idUser}})
    ...
    Promise.all([ingresos, decrements])
        ...
        .catch(err => res.status(400).json ({"message":"no se pudo realizar la consulta"}))
...
server.get('/history/time/:idUser', (req, res) => {
   ...
   var d = req.body.date
    ...
    //busco todas las transacciones del cliente y las separo por ingresos y decrementos
    let ingresos = Transactions.findAll({where: {idReceiver: req.params.idUser, createdAt :{[Op.gt]:d}}})
    let decrements = Transactions.findAll({where: {idSender: req.params.idUser, createdAt :{[Op.gt]:d}}})
